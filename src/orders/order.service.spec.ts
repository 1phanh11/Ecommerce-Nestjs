...
describe('OrderService', () => {
  ...
  afterEach(() => { return jest.clearAllMocks() })
  ...
  describe('getOneOrder', () => {
    it('Should throw not found order detail', async () => {
      ...
      await expect(orderService.getOneOrder('temp-id', 'temp-id', 'USER')).rejects.toThrow(NotFoundException)
    ...
    it('Should throw no permission to watch order', async () => {
      ...
      await expect(orderService.getOneOrder('order-1', 'user-2', 'USER')).rejects.toThrow(ForbiddenException)
    })
    ...
    it('Admin can access any user order', async () => {
      ...
      await expect(orderService.getOneOrder('order-1', 'user-admin', 'ADMIN')).resolves.toBeDefined()
    })
  ...
  describe('cancelOrder', () => {
    it("Should throw order not found exception", async () => {
      ...
      await expect(orderService.cancelOrder('temp-id', 'user-1')).rejects.toThrow(NotFoundException)
    ...
    it("Can not cancel a CANCELLED order status", async () => {
      ...
      await expect(orderService.cancelOrder('order-1', 'user-1')).rejects.toThrow(BadRequestException)
    })
    ...
    it("Cancel order success and transaction refund stock", async () => {
      ...
      const spyService = jest.spyOn(prismaService.product, 'update').mockResolvedValue({} as any)
